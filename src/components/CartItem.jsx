...
const CartItem = (props) => {
  ...
  const getItem = async () => {
    await axios
      .get(`http://localhost:3001/product/${props.id}`)
  ...
  const deleteItem = async () => {
    await axios
      .delete(`http://localhost:3001/cart/${user._id}/${props.id}`, {
  ...
  return (
    ...
    >
      <div
        style={{
          width: inNonMobileScreen ? "15%" : "100%",
          backgroundColor: "#FFF",
        }}
      >
        <img
          src={
            product && `http://localhost:3001/assets/${product.images[0].name}`
          }
          width="100%"
          alt="cartItem image"
        />
      ...
      <div
        style={{
          display: "grid",
          gridTemplateColumns: inNonMobileScreen
      ...
      >
        {product && (
          <div>
            ...
            <Typography
              variant="h5"
              sx={{ color: palette.neutral.main, m: ".5rem 0" }}
            >
              {product.category}
            </Typography>
        ...
        <div>
          <Typography
            variant="h5"
            sx={{ color: palette.neutral.main, mb: ".5rem" }}
          >
            quantity
          </Typography>
          <Typography variant="h4" sx={{ color: palette.primary.dark }}>
