...
const EditProduct = () => {
  ...
  const getSubCategories = async () => {
    await axios
      ...
      .then((res) => setSubCategoryOptions(res.data.subCategories));
  };
  ...
  const {
    ...
    onSubmit: async (values, onSubmitProps) => {
      ...
      await axios
        .patch(`http://localhost:3001/product/${id}`, formData, {
  ...
  return (
    <Box component={Paper} p="1rem">
      ...
      {submitMsg && (
        <Alert error="successful" sx={{ mb: ".5rem" }}>
          {submitMsg}
        </Alert>
      )}
      <form onSubmit={handleSubmit}>
        <div
          style={{
            display: "grid",
            gridTemplateColumns: isNonMobielScreen
